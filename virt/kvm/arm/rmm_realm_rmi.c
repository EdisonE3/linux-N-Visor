...
static void set_params_vmid_and_vcpuid(u32 sec_vm_id, u32 vcpu_id){
	// request shared memory
	unsigned int core_id;
	kvm_smc_req_t *smc_req;
	core_id = smp_processor_id();
...
u64 realm_rec_create(realm *realm_vm){
	...
	for (i = 0; i < realm_vm->rec_count; i++) {
	...
	for (j = 0UL;
	     j < (sizeof(rec_params->gprs) / sizeof(rec_params->gprs[0]));
	     j++) {
	...
	/* Create REC  */
	set_params_vmid_and_vcpuid(realm_vm->vmid, i);
